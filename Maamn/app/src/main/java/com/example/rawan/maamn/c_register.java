...
public class c_register extends AppCompatActivity {
    ...
    String username, userdisplay, useremail, userpassword, userpassword2, pname,page ,number;
    Button button3, button;
    EditText editText4, editText3, editText9, editText11, editText, editText7, editText6, editText5, editText8;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        textView7.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                Intent intent = new Intent(c_register.this, login.class);
        ...
        button3.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                pname = editText5.getText().toString();
                page = editText6.getText().toString();
                number = editText8.getText().toString();

                if (isValid(userdisplay, username, useremail, userpassword, userpassword2, number, pname, page)) {
//            //database connection
                    ...
                    try {
                        ...
                        if(isthere>0||ishere2>0){
                            Toast errorToast = Toast.makeText(c_register.this, "اسم المستخدم مسجل مسبقاً", Toast.LENGTH_SHORT);
                            errorToast.show();
                        }

                        else {
                            userpassword=md5(userpassword);
                            String sql = "INSERT INTO `caregiver` (`caregiver_ID`,`name`, `user_name`, `email`, `password`,`phoneNumber`, `patient_name`, `patient_age`) VALUES (NULL,'" + userdisplay + "', '" + username + "', '" + useremail + "', '" + userpassword + "','" + number + "','" + pname + "','" + page + "')";
                            int result = stmt.executeUpdate(sql);
                            // Toast test = Toast.makeText(p_register.this, "\n" + number, Toast.LENGTH_SHORT);
                            // test.show();
                            if (result == 1) {
                                Toast done = Toast.makeText(c_register.this, " تمت عملية التسجيل بنجاح ", Toast.LENGTH_SHORT);
                                done.show();
                                finish();
                                Intent intent = new Intent(c_register.this, caregiverHome.class);
                                startActivity(intent);
                            } else {
                                Toast done = Toast.makeText(c_register.this, " عذرًاً" + "\n" + "حدثت مشكلة حاول مجدداً ", Toast.LENGTH_SHORT);
                                done.show();
                            }
                            stmt.close();
                            con.close();
                        ...
                        Toast errorToast = Toast.makeText(c_register.this, "الرجاء التأكد من اتصال الانترنت!", Toast.LENGTH_SHORT);
                        errorToast.show();
                        se.printStackTrace();
                    } catch (Exception e) {
                        Toast errorToast = Toast.makeText(c_register.this, " " + e.getMessage(), Toast.LENGTH_SHORT);
                        errorToast.show();
                    }
    ...
    public boolean isValid(String userdisplay , String username , String useremail , String userpassword , String userpassword2, String number, String pname, String page ) {
        //validate all inputs
        if (username.equals("")) {
        ...
        if (page.equals("")){
            editText6.setError("يجب ملئ الخانة");
            return false;
        }
        if (number.equals("")){
        ...
        if(userpassword.length()<8){
            editText11.setError("يجب ان تتكون كلمة المرور من ثمانية احرف او اكثر");
            return false;
        }
        if(number.length()==11){
